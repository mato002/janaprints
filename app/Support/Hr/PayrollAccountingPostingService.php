    ...
    public function postPayrollRun(PayrollRun $run, int $userId): Journal
    {
        return $this->posting->postEvent(
            ...
            [
                ...
                'nssf_amount' => (float) $run->nssf_total + (float) $run->employer_nssf_total,
                'housing_levy_amount' => (float) $run->housing_levy_total + (float) $run->employer_housing_levy_total,
                'net_amount' => (float) $run->net_total,
                'employer_nssf_amount' => (float) $run->employer_nssf_total,
                'employer_shif_amount' => (float) $run->employer_shif_total,
                'employer_housing_levy_amount' => (float) $run->employer_housing_levy_total,
            ],
            $run->branch_id,
