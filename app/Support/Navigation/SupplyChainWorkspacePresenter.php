...
class SupplyChainWorkspacePresenter
{
    ...
    {
        return config('supply_chain_workspaces.hub', []);
    }
    ...
    {
        return config('supply_chain_workspaces.sections', []);
    }
    ...
    public function presentHub(): ?array
    {
        $items = array_map(
            ...
            $this->filterItems($this->hubDefinitions()),
        );
        ...
        return [
            ...
            'description' => __('Catalogue, store operations, inventory, procurement, costing, and reporting.'),
            'icon' => 'cube',
            'items' => $items,
        ];
    }
    ...
    public function collectActiveRoutes(): array
    {
        ...
        foreach ($this->sectionDefinitions() as $section => $definition) {
            $routes[] = "admin.workspaces.supply-chain.section:{$section}";
    ...
    {
        $flat = [];
        $workspaceTitle = __('Supply Chain');
