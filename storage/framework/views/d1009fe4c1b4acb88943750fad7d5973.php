    ...
    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('linkArtwork', $quotation)): ?>
                ...
                <?php if(count($artworkLink['library']) > 0): ?>
                    <div x-show="source === 'library'" x-cloak>
                        ...
                        <select name="customer_artwork_id" class="erp-input w-full max-w-xl" :required="source === 'library'">
                            ...
                            <?php $__currentLoopData = $artworkLink['library']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($item['id']); ?>"><?php echo e($item['label']); ?> — <?php echo e($item['type']); ?> <?php if($item['uploaded_at']): ?> (<?php echo e($item['uploaded_at']); ?>) <?php endif; ?></option>
        ...
        <?php elseif($artworkLink['can_link']): ?>
            <div class="rounded-lg border border-dashed border-erp-border p-4 text-sm text-slate-600">
                ...
                <?php if($quotation->customer): ?>
                    <a href="<?php echo e(route('admin.crm.customers.show', ['customer' => $quotation->customer, 'tab' => 'print-specifications'])); ?>" class="mt-2 inline-block text-erp-accent"><?php echo e(__('Add artwork via print specifications')); ?></a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    <?php endif; ?>
