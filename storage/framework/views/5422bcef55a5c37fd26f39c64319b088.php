...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['filters', 'branches', 'salespersons']));
...
foreach (array_filter((['filters', 'branches', 'salespersons']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.index-toolbar','data' => ['action' => route('admin.commercial.reports.customers.index'),'resetUrl' => route('admin.commercial.reports.customers.index')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['action' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('admin.commercial.reports.customers.index')),'reset-url' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('admin.commercial.reports.customers.index'))]); ?>
        ...
        <select id="customer_type" name="customer_type" class="erp-toolbar-select" aria-label="<?php echo e(__('Customer type')); ?>">
            <option value=""><?php echo e(__('All types')); ?></option>
            <?php $__currentLoopData = CustomerType::cases(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($type->value); ?>" <?php if(($filters['customer_type'] ?? '') === $type->value): echo 'selected'; endif; ?>><?php echo e(ucfirst($type->value)); ?></option>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
        ...
        <select id="activity_status" name="activity_status" class="erp-toolbar-select" aria-label="<?php echo e(__('Activity status')); ?>">
            <option value=""><?php echo e(__('All activity')); ?></option>
            <option value="active" <?php if(($filters['activity_status'] ?? '') === 'active'): echo 'selected'; endif; ?>><?php echo e(__('Active (ordered in period)')); ?></option>
            <option value="inactive" <?php if(($filters['activity_status'] ?? '') === 'inactive'): echo 'selected'; endif; ?>><?php echo e(__('Inactive (no orders in period)')); ?></option>
            <option value="new" <?php if(($filters['activity_status'] ?? '') === 'new'): echo 'selected'; endif; ?>><?php echo e(__('New (created in period)')); ?></option>
            <option value="dormant" <?php if(($filters['activity_status'] ?? '') === 'dormant'): echo 'selected'; endif; ?>><?php echo e(__('Dormant (no recent orders)')); ?></option>
        </select>
        <input
            type="search"
            id="search"
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.status-pills','data' => ['options' => collect(CustomerStatus::cases())->map(fn ($status) => ['value' => $status->value, 'label' => ucfirst($status->value)])->prepend(['value' => '', 'label' => __('All')])->all(),'param' => 'status','current' => $filters['status'] ?? '']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['options' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(collect(CustomerStatus::cases())->map(fn ($status) => ['value' => $status->value, 'label' => ucfirst($status->value)])->prepend(['value' => '', 'label' => __('All')])->all()),'param' => 'status','current' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($filters['status'] ?? '')]); ?>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\jana-prints\resources\views\admin\commercial\reports\customers\partials\filters.blade.php ENDPATH**/ ?>