...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.card','data' => ['class' => 'mb-4','id' => 'upload-file']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['class' => 'mb-4','id' => 'upload-file']); ?>
    <h3 class="mb-3 font-medium"><?php echo e(__('Upload file')); ?></h3>
    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('update', $customer)): ?>
        <form method="POST" action="<?php echo e(route('admin.crm.customers.files.store', $customer)); ?>" enctype="multipart/form-data" data-turbo-frame="erp-main" class="flex flex-wrap items-end gap-3">
            <?php echo csrf_field(); ?>
            <input type="file" name="file" class="text-sm" required>
            <button type="submit" class="erp-btn-primary text-sm"><?php echo e(__('Upload')); ?></button>
        </form>
    <?php else: ?>
        <p class="text-sm text-slate-500"><?php echo e(__('You do not have permission to upload files.')); ?></p>
    <?php endif; ?>
     ...
     <?php $__env->slot('body', null, []); ?> 
        <?php $__empty_1 = true; $__currentLoopData = $files; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr>
                ...
                <td><?php echo e($file->uploader?->name ?? '—'); ?></td>
                <td class="tabular-nums"><?php echo e(number_format($file->size / 1024, 1)); ?> KB</td>
                <td><?php echo e($file->created_at?->format('Y-m-d H:i')); ?></td>
                <td class="text-end">
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('update', $customer)): ?>
                        <form method="POST" action="<?php echo e(route('admin.crm.customers.files.destroy', [$customer, $file])); ?>" class="inline" data-turbo-confirm="<?php echo e(__('Remove this file?')); ?>">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="submit" class="text-xs text-red-600 hover:text-red-700"><?php echo e(__('Remove')); ?></button>
                        </form>
                    <?php endif; ?>
                </td>
            </tr>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <tr><td colspan="5" class="py-6 text-center text-slate-500"><?php echo e(__('No files uploaded yet.')); ?></td></tr>
        <?php endif; ?>
     <?php $__env->endSlot(); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.table-pagination','data' => ['paginator' => $files]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['paginator' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($files)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\jana-prints\resources\views\admin\crm\customers\workspace\tabs\files.blade.php ENDPATH**/ ?>