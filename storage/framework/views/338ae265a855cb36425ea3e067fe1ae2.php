    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('production.outputs.post')): ?>
                <?php if(! $onOutputsTab && (($completion['eligible'] ?? false) || ($finishedItems ?? collect())->isNotEmpty())): ?>
                    <?php if($eligible): ?>
                        <button type="button" class="erp-btn-secondary text-sm" data-open-dialog="complete-fg-modal"><?php echo e($postLabel); ?></button>
                    <?php else: ?>
                        <button type="button" class="erp-btn-secondary text-sm opacity-60" disabled>
                            <?php echo e(__('Post to finished goods')); ?>

                        </button>
                        <?php if($remaining > 0): ?>
                            <span class="text-xs text-amber-700"><?php echo e(trans_choice(':count requirement remaining|:count requirements remaining', $remaining, ['count' => $remaining])); ?></span>
                        <?php endif; ?>
                    <?php endif; ?>
                    <a
                        href="<?php echo e(route('admin.production.job-cards.show', ['jobCard' => $jobCard, 'tab' => 'outputs'])); ?>"
                        class="erp-link text-sm self-center"
                        data-turbo-frame="erp-main"
                    ><?php echo e(__('Finished Goods')); ?></a>
                <?php endif; ?>
            <?php endif; ?>
