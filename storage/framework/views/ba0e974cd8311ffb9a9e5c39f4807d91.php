...
>
    <div x-show="panelLoading" class="rounded-xl border border-erp-border bg-white px-6 py-16 text-center text-sm text-slate-500">
        <?php echo e(__('Loading workspace…')); ?>
            ...
            <div class="overflow-hidden rounded-xl border border-violet-200 bg-white shadow-lg">
                ...
                <div class="border-b border-violet-100 bg-gradient-to-r from-violet-50 via-white to-white px-5 py-4">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-bold uppercase tracking-widest text-violet-600"><?php echo e(__('Working on')); ?></p>
                            <h2 class="font-mono text-2xl font-bold text-erp-primary" x-text="panel.header?.request_number"></h2>
                        </div>
                        <button type="button" class="erp-btn-secondary text-sm" @click="clearSelection()"><?php echo e(__('Back to queue')); ?></button>
                    </div>
                    <dl class="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-4">
                        <div>
                            <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-400"><?php echo e(__('Customer')); ?></dt>
                            <dd class="mt-0.5 font-semibold text-slate-900" x-text="panel.context?.customer ?? '—'"></dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-400"><?php echo e(__('Product')); ?></dt>
                            <dd class="mt-0.5 font-medium text-slate-800" x-text="panel.context?.product ?? panel.header?.title ?? '—'"></dd>
                        </div>
                        <div>
                            <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-400"><?php echo e(__('Due')); ?></dt>
                            <dd
                        ...
                        <div>
                            <dt class="text-[10px] font-semibold uppercase tracking-wide text-slate-400"><?php echo e(__('Priority')); ?></dt>
                            <dd class="mt-0.5">
                                <span
                ...
                <div class="grid grid-cols-1 gap-0 lg:grid-cols-2">
                    ...
                    <section id="designer-desk-files" class="border-b border-erp-border p-5 lg:border-b-0 lg:border-r">
                        <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500"><?php echo e(__('Customer Files')); ?></h3>
                        <template x-if="!(panel.files?.customer?.length)">
                            <p class="mb-3 text-sm text-slate-500"><?php echo e(__('No files uploaded yet.')); ?></p>
                        </template>
                        <ul class="mb-4 space-y-1.5">
                            <template x-for="file in panel.files?.customer ?? []" :key="file.id">
                                <li class="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm">
                                    ...
                                    <a x-show="file.download_url" :href="file.download_url" class="text-xs font-semibold text-erp-accent hover:underline" download><?php echo e(__('Download')); ?></a>
                                </li>
                            </template>
                        </ul>
                        ...
                        <div id="designer-desk-upload">
                            <template x-if="panel.files?.can_upload_version">
                                ...
                                >
                                    ...
                                    <?php endif; ?>
                                    <p class="mb-2 text-sm font-semibold text-violet-900">+ <?php echo e(__('Upload Artwork')); ?></p>
                                    <input type="file" name="file" class="erp-input mb-2 w-full" accept=".pdf,.ai,.psd,.cdr,.svg,.png,.jpg,.jpeg" required>
                                    <input type="text" name="notes" class="erp-input mb-2 w-full text-sm" placeholder="<?php echo e(__('Version notes')); ?>">
                                    <button type="submit" class="erp-btn-primary w-full text-sm"><?php echo e(__('Upload version')); ?></button>
                                </form>
                            </template>
                        </div>
                    </section>
                    ...
                    <section class="border-b border-erp-border p-5">
                        <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500"><?php echo e(__('Specifications')); ?></h3>
                    ...
                    <section class="border-b border-erp-border p-5 lg:border-b-0 lg:border-r">
                        <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500"><?php echo e(__('Version History')); ?></h3>
                        <template x-if="!(panel.files?.versions?.length)">
                            <p class="text-sm text-slate-500"><?php echo e(__('No versions yet.')); ?></p>
                        </template>
                    ...
                    <section class="border-b border-erp-border p-5">
                        <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500"><?php echo e(__('Revision Notes')); ?></h3>
                        <div class="space-y-3 text-sm">
                            <template x-if="panel.revision_notes?.customer?.length">
                                <div>
                                    <p class="mb-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700"><?php echo e(__('Customer')); ?></p>
                            ...
                            <template x-if="panel.revision_notes?.sales?.length">
                                <div>
                                    <p class="mb-1 text-[10px] font-semibold uppercase tracking-wide text-blue-700"><?php echo e(__('Sales')); ?></p>
                            ...
                            <template x-if="panel.revision_notes?.internal?.length">
                                <div>
                                    <p class="mb-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500"><?php echo e(__('Internal')); ?></p>
                            ...
                            <template x-if="!(panel.revision_notes?.customer?.length || panel.revision_notes?.sales?.length || panel.revision_notes?.internal?.length)">
                                <p class="text-slate-500"><?php echo e(__('No revision notes.')); ?></p>
                            </template>
                        </div>
                    </section>
                    ...
                    <section class="col-span-1 p-5 lg:col-span-2 lg:border-t lg:border-erp-border">
                        <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500"><?php echo e(__('Production Checklist')); ?></h3>
