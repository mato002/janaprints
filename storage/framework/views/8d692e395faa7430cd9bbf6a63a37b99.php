...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.lookup-select','data' => ['name' => 'sales_order_id','label' => __('Sales order'),'options' => $lookupOptions,'value' => old('sales_order_id', $preselectedSalesOrderId ?? null),'required' => true,'createRoute' => 'admin.sales-orders.create','createQuery' => array_filter([
                            'tab' => 'direct',
                            'from' => ($fromProductionFloor ?? false) ? 'production-floor' : null,
                        ]),'refreshRoute' => 'admin.lookups.job_card_sales_orders','permission' => 'sales_orders.create','modalTitle' => __('Create sales order'),'placeholder' => __('Select sales order')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'sales_order_id','label' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(__('Sales order')),'options' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($lookupOptions),'value' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(old('sales_order_id', $preselectedSalesOrderId ?? null)),'required' => true,'create-route' => 'admin.sales-orders.create','create-query' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(array_filter([
                            'tab' => 'direct',
                            'from' => ($fromProductionFloor ?? false) ? 'production-floor' : null,
                        ])),'refresh-route' => 'admin.lookups.job_card_sales_orders','permission' => 'sales_orders.create','modal-title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(__('Create sales order')),'placeholder' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(__('Select sales order'))]); ?>
                            ...
                            <?php if(($summary['already_have_job'] ?? 0) === 0 && ($summary['blocked_artwork'] ?? 0) === 0): ?>
                                <p class="mt-2 text-xs">
                                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sales_orders.create')): ?>
                                        <?php echo e(__('No confirmed orders yet. Use + beside Sales order to create one, then check again.')); ?>

                                    <?php else: ?>
                                        <?php echo e(__('No confirmed orders yet. Ask Sales to create and release an order, then check again.')); ?>
