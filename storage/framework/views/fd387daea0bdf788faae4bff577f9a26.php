...
<?php $component->withAttributes(['action' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(route('admin.sales-orders.store')),'class' => 'space-y-4']); ?>
                ...
                <?php if(($fields['quotation_id']['visible'] ?? true)): ?>
                    <div>
                        <label class="erp-label" for="quotation_id"><?php echo e(__('Quotation')); ?></label>
                        <select id="quotation_id" name="quotation_id" class="erp-select w-full min-h-[2.75rem]" <?php if($fields['quotation_id']['required'] ?? true): echo 'required'; endif; ?>>
                            <option value=""><?php echo e(__('Select quotation')); ?></option>
                            <?php $__currentLoopData = $eligibleQuotations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quotation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($quotation->id); ?>" <?php if(old('quotation_id', $selectedQuotationId ?? null) == $quotation->id): echo 'selected'; endif; ?>>
                                    <?php echo e($quotation->salesOrderConversionLabel()); ?>

                                </option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                        <?php if($eligibleQuotations->isEmpty()): ?>
                            <p class="mt-2 text-sm text-slate-500"><?php echo e(__('No accepted quotations are available. Accept a quotation first, or use Direct Order.')); ?></p>
                        <?php elseif($eligibleQuotations->every(fn ($quotation) => ! $quotation->isReadyForSalesOrderConversion())): ?>
                            <p class="mt-2 text-sm text-amber-700"><?php echo e(__('Selected quotations still need approved artwork before conversion.')); ?></p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
...
<?php $component->withAttributes(['class' => 'erp-form-modal__actions--sticky']); ?>
                    <button type="submit" class="erp-btn-primary min-h-[2.75rem] w-full sm:w-auto" <?php if($eligibleQuotations->isEmpty()): echo 'disabled'; endif; ?>><?php echo e(__('Create from quotation')); ?></button>
...
<?php endif; ?>
                ...
                <template x-if="customerId && !loadingContext && !contextError && context && !context.print_specifications?.length">
                    <p class="text-sm text-slate-500"><?php echo e(__('No active print specifications for this customer.')); ?></p>
                </template>

                <template x-if="context && !loadingContext">
                    <div class="space-y-4">
                        <div>
                            <h3 class="mb-2 text-sm font-semibold text-slate-900"><?php echo e(__('Print specifications')); ?></h3>
                                ...
                                <table class="erp-table w-full text-sm">
                                    ...
                                    <tbody>
                                        ...
                                        <tr x-show="!context.print_specifications?.length">
                                            <td colspan="6" class="py-6 text-center text-slate-500"><?php echo e(__('No active print specifications for this customer.')); ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
