<script>
    function posCounterWorkstation(config) {
        return {
            ...
            get customerLabel() {
                if (this.walkIn) return <?php echo json_encode(__('Walk-in customer'), 15, 512) ?>;
                const match = this.customers.find(c => String(c.id) === String(this.customerId));
                return match?.company_name ?? <?php echo json_encode(__('Select customer'), 15, 512) ?>;
            },
            ...
            applyHeldCart(cart) {
                ...
                this.resumeLabel = <?php echo json_encode(__('Resuming'), 15, 512) ?> + ' ' + cart.sale_number;
            },

            async cancelHeld(hold) {
                if (!confirm(<?php echo json_encode(__('Cancel this held sale?'), 15, 512) ?>)) return;
            ...
            clearCart(confirm = true) {
                if (confirm && !window.confirm(<?php echo json_encode(__('Clear the current sale?'), 15, 512) ?>)) return;
                this.lines = [];
                this.saleDiscount = 0;
                this.saleTax = 0;
                this.paymentMethod = '';
            ...
            async submitPayment() {
                if (!this.paymentMethod) { this.paymentError = <?php echo json_encode(__('Select a payment method.'), 15, 512) ?>; return; }
                this.loading = true;
                this.paymentError = '';
                try {
...
<?php /**PATH C:\xampp\htdocs\jana-prints\resources\views\admin\commercial\pos\partials\counter-sales-script.blade.php ENDPATH**/ ?>