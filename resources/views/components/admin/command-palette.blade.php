...
>
    ...
    <div
        ...
        @click.stop
    >
        <h2 id="erp-command-palette-title" class="sr-only">{{ __('Global feature finder') }}</h2>
        ...
        <div class="border-b border-erp-border px-4 py-3">
            <div class="relative">
                ...
                <input
                    ...
                    placeholder="{{ __('Search customers, jobs, reports, settings, features…') }}"
                    autocomplete="off"
                    aria-label="{{ __('Search ERP features') }}"
                >
        ...
        <div class="max-h-[min(60vh,28rem)] overflow-y-auto py-2" x-ref="paletteResults">
            <template x-if="paletteQuery.trim() === '' && recentItems.length > 0">
                <section class="px-2 pb-2">
                    <p class="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">{{ __('Recent') }}</p>
                    <template x-for="(item, index) in recentItems" :key="`recent-${item.id}`">
                        ...
                        >
                            ...
                            <div class="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                <button type="button" class="erp-btn erp-btn--ghost erp-btn--xs" @click="openPaletteItemNewTab(item)" title="{{ __('Open in new tab') }}">
                                    <x-admin.icon name="external-link" class="h-3.5 w-3.5" />
                                </button>
                                <button type="button" class="erp-btn erp-btn--ghost erp-btn--xs" @click="copyPaletteItemLink(item)" title="{{ __('Copy link') }}">
                                    <x-admin.icon name="clipboard" class="h-3.5 w-3.5" />
                                </button>
                                <button type="button" class="erp-btn erp-btn--ghost erp-btn--xs" @click="toggleDiscoveryFavorite(item.id)" title="{{ __('Favorite') }}">
            ...
            <template x-if="paletteQuery.trim() === '' && favoriteDiscoveryItems.length > 0">
                <section class="px-2 pb-2">
                    <p class="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-500">{{ __('Favorites') }}</p>
        ...
        <div class="flex items-center justify-between border-t border-erp-border bg-erp-page/60 px-4 py-2 text-[11px] text-slate-500">
            <div class="flex items-center gap-3">
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-erp-border px-1">↑</kbd><kbd class="rounded border border-erp-border px-1">↓</kbd> {{ __('Navigate') }}</span>
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-erp-border px-1">↵</kbd> {{ __('Open') }}</span>
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-erp-border px-1">Esc</kbd> {{ __('Close') }}</span>
            </div>
            <span>{{ __('Feature search — not record search') }}</span>
        </div>
    </div>
</div>
