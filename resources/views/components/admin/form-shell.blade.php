...
<form
    ...
    action="{{ $action }}"
    @if ($inFormModal) data-turbo="false" @else data-turbo-frame="erp-main" data-turbo-action="advance" @endif
    {{ $attributes->merge(['class' => 'erp-form-shell']) }}
>
    @csrf
    @if ($inFormModal)
        <input type="hidden" name="_erp_modal" value="1">
        <input type="hidden" name="_erp_modal_form_url" value="{{ url()->current() }}">
