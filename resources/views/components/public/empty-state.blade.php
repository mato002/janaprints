...
<div {{ $attributes->merge(['class' => 'public-empty-state']) }} role="status">
    ...
    <h3 class="public-empty-state__title">{{ $title }}</h3>
    <p class="public-empty-state__desc">{{ $description }}</p>
    @if ($slot->isNotEmpty())
        <div class="public-empty-state__action">{{ $slot }}</div>
    @endif
</div>
