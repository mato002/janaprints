    ...
    <article class="public-card public-card--soft p-6 sm:p-8" data-animate="fade-up" data-contact-location-fallback>
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="mt-2 text-sm font-medium text-brand-navy">{{ $contact['address'] }}</p>
                @if (! empty($contact['address_detail']))
                    <p class="mt-1 text-sm text-brand-text-secondary">{{ $contact['address_detail'] }}</p>
                @endif
                @if (! empty($contact['map_placeholder']))
                    <p class="mt-2 text-xs text-brand-text-muted">{{ $contact['map_placeholder'] }}</p>
                @endif
                @if (! empty($contact['map_url']))
                    <a href="{{ $contact['map_url'] }}" target="_blank" rel="noopener noreferrer" class="public-link mt-4 inline-flex text-sm">
                        Open in Google Maps
                    </a>
                @endif
            </div>
