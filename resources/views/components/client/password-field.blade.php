...
<div class="client-form__group">
    <label for="{{ $id }}" class="client-label">{{ $label }}</label>
    <div class="client-password-wrap">
        <input
            type="password"
            name="{{ $name }}"
            id="{{ $id }}"
            ...
            {{ $attributes->merge(['class' => 'client-input client-input--password']) }}
        >
        <button
            type="button"
