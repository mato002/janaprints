...
<div
    x-data="{
        artworks: [],
        customerArtworkId: @js((string) old('customer_artwork_id', '')),
        loading: false,
        async loadArtworks(customerId) {
            ...
            this.loading = true;
            try {
                const response = await fetch(`${@js($artworksUrl)}/${customerId}/artworks`, {
...
>
    <label class="erp-label" for="customer_artwork_id">{{ __('Artwork') }}</label>
    <select
