    ...
    <x-admin.form-shell :action="route('admin.quotations.update', $quotation)" method="PUT" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="valid_until" class="erp-input" min="{{ now()->toDateString() }}" value="{{ old('valid_until', $quotation->valid_until?->format('Y-m-d')) }}" @required($fields['valid_until']['required'] ?? false) @readonly($fields['valid_until']['read_only'] ?? false)>
            </div>
            @endif
