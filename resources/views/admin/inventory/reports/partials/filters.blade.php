@props(['filters', 'branches', 'warehouses', 'categories', 'suppliers', 'items'])

<x-admin.card class="mb-6">
    <form method="GET" action="{{ route('admin.inventory.reports.index') }}" data-turbo-frame="erp-main" class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <input type="hidden" name="tab" value="{{ $filters['tab'] ?? 'stock_on_hand' }}">
        ...
        <div>
            <label class="text-[11px] text-slate-500" for="item_id">{{ __('Item') }}</label>
            <select id="item_id" name="item_id" class="erp-input mt-1 w-full">
                <option value="">{{ __('All items') }}</option>
                @foreach ($items as $item)
                    <option value="{{ $item->id }}" @selected(($filters['item_id'] ?? null) == $item->id)>{{ $item->item_name }} ({{ $item->sku }})</option>
                @endforeach
            </select>
        </div>
        <div class="md:col-span-2">
            ...
            <input
                ...
                placeholder="{{ __('Item name or SKU…') }}"
