...
<<<<<<< Updated upstream
    ...
    >
=======
    <x-admin.page-header :title="__('Price Lists')"><x-slot name="actions">@if (auth()->user()?->can('catalogue.create'))<a href="{{ route('admin.inventory.catalogue.price-lists.create') }}" class="erp-btn-primary" data-erp-modal-open>{{ __('New Price List') }}</a>@endif</x-slot></x-admin.page-header>
    <x-admin.data-table :search-placeholder="__('Search price lists...')" export-filename="price-lists">
>>>>>>> Stashed changes
