...
<x-admin.data-table :search-placeholder="__('Search SKU or item name…')" export-filename="reorder-alerts">
    <x-slot name="filters">
        <form method="GET" action="{{ $filterAction }}" x-data="erpIndexFilterForm()" @change="onFieldChange($event)" class="flex flex-wrap items-center gap-2" data-turbo-frame="{{ $frame }}" data-turbo-action="advance">
            ...
            @endif
            <a href="{{ $resetUrl }}" class="erp-btn-ghost py-1 text-xs text-slate-500" data-turbo-frame="{{ $frame }}" data-turbo-action="advance">{{ __('Reset') }}</a>
        </form>
    </x-slot>
    <x-slot name="head">
        <tr>
