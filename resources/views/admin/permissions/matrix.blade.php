...
>
    ...
    <x-admin.card class="mb-3 !px-4 !py-3">
        <form method="GET" action="{{ route('admin.access-control.matrix') }}" class="flex flex-wrap items-end gap-3">
            ...
            @if ($selectedRole)
                <a href="{{ route('admin.roles.show', $selectedRole) }}" class="erp-btn-secondary !px-3 !py-1.5 text-xs" data-turbo-action="advance">{{ __('Edit access rights') }}</a>
            @endif
        </form>
    </x-admin.card>
