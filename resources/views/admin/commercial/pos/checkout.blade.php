<x-admin-layout :title="__('Resume :number', ['number' => $heldSale->sale_number])" :breadcrumbs="[['label' => __('POS'), 'url' => route('admin.commercial.pos.dashboard')], ['label' => __('Held sales'), 'url' => route('admin.commercial.pos.holds')], ['label' => $heldSale->sale_number]]">
    @if (empty($activeSession))
        <div class="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">
            {{ __('Open a POS session before completing held sales.') }}
    ...
    <div class="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        {{ __('Resuming held sale :number — held :time by :cashier.', [
            'number' => $heldSale->sale_number,
            'time' => $heldSale->hold?->held_at?->format('Y-m-d H:i') ?? $heldSale->updated_at->format('Y-m-d H:i'),
            'cashier' => $heldSale->cashier?->name ?? __('Unknown'),
        ]) }}
    </div>

    <form method="POST" action="{{ route('admin.commercial.pos.pay', $heldSale) }}" x-data="posCart(@js($items->map(fn ($i) => ['id' => $i->id, 'name' => $i->item_name, 'price' => (float) $i->standard_cost])->values()), @js($heldCart))" @if (empty($activeSession)) class="pointer-events-none opacity-60" @endif>
        @csrf
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-4">
                ...
                <x-admin.card>
                    ...
                    <div class="mt-4">
                        <label class="text-sm">{{ __('Payment method') }} <span class="text-red-600">*</span></label>
                        <select name="payment_method" class="erp-input mt-1 w-full" required>
                    ...
                    <div class="mt-4 flex flex-col gap-2">
                        <button type="submit" class="erp-btn-primary w-full">{{ __('Complete & pay held sale') }}</button>
                        <a href="{{ route('admin.commercial.pos.holds') }}" class="erp-btn-secondary w-full text-center">{{ __('Back to held queue') }}</a>
                    </div>
                </x-admin.card>
            </div>
        </div>
    ...
    <script>
        function posCart(catalog, held = null) {
            const defaults = held ?? {
            ...
            return {
                catalog,
                lines: defaults.lines.length ? defaults.lines : [{ item_id: '', description: '', quantity: 1, unit_price: 0, discount_amount: 0, tax_amount: 0 }],
                saleDiscount: defaults.saleDiscount ?? 0,
                saleTax: defaults.saleTax ?? 0,
                walkIn: defaults.walkIn ?? true,
