...
<div x-show="showOpenSessionModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-md rounded-xl border border-erp-border bg-white p-6 shadow-xl">
        <h3 class="text-lg font-semibold">{{ __('Open POS session') }}</h3>
        <form class="mt-4 space-y-3" @submit.prevent="submitOpenSession()">
            <div>
                <label class="text-xs text-slate-500">{{ __('Cashier') }}</label>
            ...
            <div class="flex gap-2 pt-2">
                <button type="submit" class="erp-btn-primary" :disabled="loading">{{ __('Open session') }}</button>
                <button type="button" class="erp-btn-secondary" @click="showOpenSessionModal = false">{{ __('Cancel') }}</button>
            </div>
        </form>
    </div>
</div>
...
<div x-show="showPaymentModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-md rounded-xl border border-erp-border bg-white p-6 shadow-xl">
        <h3 class="text-lg font-semibold">{{ __('Complete payment') }}</h3>
        <p class="mt-1 text-sm text-slate-500">{{ __('Grand total') }}: <strong x-text="formatMoney(grandTotal)"></strong></p>
        ...
        <div class="mt-3 space-y-3">
            <div x-show="paymentMethod === 'cash'">
                ...
                <p class="mt-1 text-sm" x-show="changeDue > 0">{{ __('Change due') }}: <span class="font-medium tabular-nums" x-text="formatMoney(changeDue)"></span></p>
            </div>
            <div>
                <label class="text-xs text-slate-500">{{ __('Reference number') }}</label>
                <input type="text" class="erp-input mt-1 w-full" x-model="paymentReference">
            </div>
            <p class="text-xs text-slate-400">{{ __('Split payment extension point — multi-tender checkout will plug in here.') }}</p>
        </div>
        <p class="mt-3 text-sm text-red-600" x-text="paymentError" x-show="paymentError"></p>
        <div class="mt-4 flex gap-2">
            <button type="button" class="erp-btn-primary" @click="submitPayment()" :disabled="loading || !paymentMethod">{{ __('Complete sale') }}</button>
            <button type="button" class="erp-btn-secondary" @click="showPaymentModal = false">{{ __('Cancel') }}</button>
        </div>
    </div>
</div>
...
<div x-show="showReceiptModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl border border-erp-border bg-white p-6 shadow-xl" id="pos-receipt-modal">
        <template x-if="receipt">
            <div>
                <div class="text-center border-b border-erp-border pb-4 mb-4">
                    ...
                    <p class="text-xs text-slate-400">{{ __('Cashier') }}: <span x-text="receipt.cashier_name"></span></p>
                </div>
                <p class="text-sm mb-3">{{ __('Customer') }}: <span x-text="receipt.customer_label"></span></p>
                ...
                <div class="space-y-1 text-sm border-t border-erp-border pt-3">
                    <div class="flex justify-between"><span>{{ __('Total') }}</span><span class="font-bold tabular-nums" x-text="formatMoney(receipt.total_amount)"></span></div>
        ...
        <div class="mt-4 flex flex-wrap gap-2 print:hidden">
            <button type="button" class="erp-btn-primary" @click="printReceipt()">{{ __('Print receipt') }}</button>
            <button type="button" class="erp-btn-secondary" @click="printReceipt()" x-show="permissions.canReprint">{{ __('Reprint receipt') }}</button>
            <a :href="receipt?.full_receipt_url" target="_blank" class="erp-btn-secondary" x-show="receipt">{{ __('Open full receipt') }}</a>
            <button type="button" class="erp-btn-secondary" @click="newSale()">{{ __('New sale') }}</button>
            <button type="button" class="erp-btn-secondary" @click="showReceiptModal = false">{{ __('Close') }}</button>
        </div>
    </div>
</div>
...
<div x-show="showCustomerModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
    ...
    <div class="relative w-full max-w-md rounded-xl border border-erp-border bg-white p-6 shadow-xl">
        ...
        <input type="search" class="erp-input mt-3 w-full" placeholder="{{ __('Search customer…') }}" x-model="customerSearch" :disabled="walkIn">
        ...
        <a :href="customerCreateUrl" target="_blank" class="mt-3 inline-block text-xs text-erp-primary underline" x-show="permissions.canAddCustomer">{{ __('Add new customer') }}</a>
        <button type="button" class="erp-btn-secondary mt-4 w-full" @click="showCustomerModal = false">{{ __('Done') }}</button>
    </div>
</div>
