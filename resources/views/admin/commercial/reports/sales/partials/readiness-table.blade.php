@props(['readiness', 'report_ready', 'context' => __('sales reports')])

<x-admin.card class="mb-6">
    <div class="mb-3 flex items-center justify-between gap-3">
        <div>
            <h2 class="text-sm font-semibold text-erp-primary">{{ __('Data Readiness') }}</h2>
            <p class="mt-1 text-xs text-slate-500">{{ __('Operational sources required before :context can run.', ['context' => $context]) }}</p>
        </div>
        <span @class([
