...
@else
    <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-sm font-semibold text-erp-primary">{{ $registerTitle ?? __('Purchase Requests') }}</h2>
        </div>
        @can('create', App\Models\Procurement\PurchaseRequest::class)
            <a href="{{ route('admin.procurement.requests.create') }}" class="erp-btn-primary text-sm">{{ __('Create request') }}</a>
        @endcan
    </div>
@endif
