<x-admin-layout :title="__('Email Identity')" :breadcrumbs="[['label' => __('Administration')], ['label' => __('Integrations')], ['label' => __('Email Identity')]]">
    <x-admin.page-header
        ...
        :description="__('Outbound sender addresses and production readiness for employee onboarding.')"
    />
    ...
    @if (config('email_local_testing.enabled') && config('email_local_testing.show_admin_banner'))
        <div class="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
            ...
            {{ __('Onboarding invitations send through :address via Gmail/dev SMTP. Set EMAIL_LOCAL_TESTING=false before production.', [
...
@php
    ...
    <section class="mt-6 rounded-lg border border-gray-200 bg-white p-5">
        <h2 class="text-base font-semibold text-gray-900">{{ __('Department & system senders') }}</h2>
        <p class="mt-1 text-sm text-gray-500">{{ __('Configured FROM addresses for outbound system mail.') }}</p>
