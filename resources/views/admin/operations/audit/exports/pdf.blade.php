...
<head>
    <meta charset="UTF-8">
    <title>{{ __('Audit Logs Report') }}</title>
    <style>
        ...
        <tbody>
            @foreach ($events as $event)
                <tr>
                    ...
                    <td>{{ $event->entity ? \Illuminate\Support\Str::headline($event->entity) : '—' }}</td>
                    <td>{{ $auditService->actionLabel($event) }}</td>
                    <td>{{ $auditService->formatValues($event->before_values) }}</td>
                    <td>{{ $auditService->formatValues($event->after_values) }}</td>
