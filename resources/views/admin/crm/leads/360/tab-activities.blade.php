<div class="crm-360__tab-stack">
    @can('create', App\Models\Crm\CustomerActivity::class)
        <section class="crm-360__card crm-360__card--form">
            ...
            <form method="POST" action="{{ route('admin.crm.leads.activities.store', $lead) }}" class="crm-360__form-grid">
                @csrf
                <div>
        ...
        <div class="crm-360__card-head">
            ...
            @can('viewAny', App\Models\Crm\CustomerActivity::class)
                <x-admin.crm-btn
                    variant="outline"
                    size="sm"
                    :href="route('admin.commercial.activities.index', ['lead_id' => $lead->id])"
                    ...
                    @foreach ($group as $activity)
                        <li class="crm-360__feed-item">
                            ...
                            @if ($activity->description)
                                <p class="crm-360__feed-meta text-slate-500">{{ $activity->description }}</p>
                            @endif
                        </li>
                    @endforeach
                </ul>
