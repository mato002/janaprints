...
<div class="designer-desk-metrics mb-4 space-y-3">
    <div class="rounded-xl border border-erp-border bg-white p-3 shadow-sm">
        <p class="mb-2 px-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ __('Operational') }}</p>
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            @foreach ($summary['operational'] as $card)
    ...
    <div class="rounded-xl border border-erp-border/80 bg-gradient-to-r from-slate-50 to-white p-3">
        <p class="mb-2 px-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ __('Performance') }}</p>
        <div class="grid grid-cols-2 gap-2 lg:grid-cols-4">
            @foreach ($summary['performance'] as $card)
                <div class="rounded-lg border border-white bg-white/80 px-3 py-2.5 text-center shadow-sm">
                    <p class="text-lg font-bold tabular-nums {{ $toneClasses($card['tone'] ?? 'slate') }}">{{ $card['value'] }}</p>
                    <p class="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500">{{ $card['label'] }}</p>
                </div>
            @endforeach
        </div>
