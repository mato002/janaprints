@php
    ...
    $statusBadgeClasses = [
        ...
        'in_design' => 'bg-blue-50 text-blue-700 ring-blue-600/20',
        'submitted' => 'bg-indigo-50 text-indigo-700 ring-indigo-600/20',
        'approved' => 'bg-emerald-50 text-emerald-700 ring-emerald-600/20',
        'revision_requested' => 'bg-amber-50 text-amber-800 ring-amber-600/20',
            ...
            <div class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-none">
                ...
                <div class="border-b border-slate-200 bg-slate-50/50 px-5 py-4">
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <div class="min-w-0">
                            <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ __('Selected job') }}</p>
                        ...
                        <button type="button" class="text-xs font-medium text-slate-500 hover:text-slate-800 lg:hidden" @click="clearSelection()">{{ __('Close') }}</button>
                    </div>
                    ...
                    <dl class="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                        <div>
                            ...
                            <dd class="font-medium text-slate-800" x-text="panel.header?.designer || '{{ __('Unclaimed') }}'"></dd>
                        </div>
                        <div>
                        ...
                        <div>
                            <dt class="text-[10px] uppercase tracking-wide text-slate-400">{{ __('Priority') }}</dt>
                ...
                <template x-if="panel.guidance">
                    <div class="border-b border-erp-accent/15 bg-gradient-to-r from-erp-accent/[0.06] to-transparent px-5 py-3">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-500">{{ __('Workflow') }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3">
                    <div class="space-y-0 border-b border-slate-200 lg:col-span-2 lg:border-b-0 lg:border-r">
                        ...
                        <section id="designer-desk-files" class="border-b border-slate-200 p-5">
                            <h3 class="artwork-detail-card__title">{{ __('Artwork & files') }}</h3>

                            <p class="mb-2 text-[10px] font-semibold uppercase text-slate-400">{{ __('Versions') }}</p>
                                ...
                                <template x-if="panel.files?.can_upload_version">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <p class="artwork-detail-file-upload__label" x-show="!fileName">{{ __('Choose artwork file') }}</p>
                                            <p class="artwork-detail-file-upload__hint" x-show="!fileName">{{ __('PDF, AI, PSD, PNG, JPG…') }}</p>
                                            <p class="artwork-detail-file-upload__name" x-show="fileName" x-text="fileName"></p>
                                        </div>
                                        <input type="text" name="notes" class="erp-input w-full text-sm" placeholder="{{ __('Version notes') }}">
                                        <button
                                            type="submit"
                                            class="erp-btn-primary w-full text-sm"
                                            :class="{ 'erp-btn-secondary': (panel.files?.versions?.length ?? 0) > 0 }"
                                        >{{ __('Upload version') }}</button>
                                    </form>
                                </template>
                            </div>
                            ...
                            <template x-if="panel.files?.customer?.length">
                                <div class="mt-4 border-t border-slate-100 pt-4">
                                    <p class="mb-2 text-[10px] font-semibold uppercase text-slate-400">{{ __('Customer files') }}</p>
                                    <ul class="space-y-1">
                                        <template x-for="file in panel.files?.customer ?? []" :key="file.id">
                                            <li class="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs">
                                                ...
                                                <a x-show="file.download_url" :href="file.download_url" class="shrink-0 font-semibold text-erp-accent" download>{{ __('Download') }}</a>
                                            </li>
                                        </template>
                                    </ul>
                                </div>
                        ...
                        <section class="border-b border-slate-200 p-5">
                            <h3 class="artwork-detail-card__title">{{ __('Specification') }}</h3>
                        ...
                        <section class="p-5">
                            <h3 class="artwork-detail-card__title">{{ __('Notes & comments') }}</h3>
                            <div class="space-y-2 text-sm">
                                ...
                                <template x-if="!(panel.revision_notes?.customer?.length || panel.revision_notes?.sales?.length || panel.revision_notes?.internal?.length)">
                                    <p class="text-slate-500">{{ __('No notes yet.') }}</p>
                                </template>
                            </div>
                        </section>
                    </div>
                    ...
                    <aside class="flex flex-col bg-slate-50/40 p-5">
                        <h3 class="artwork-detail-card__title">{{ __('Actions') }}</h3>
                        ...
                        <template x-if="panel.links?.length">
                            <div class="mt-4 border-t border-slate-200 pt-3">
                                <p class="mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ __('Open') }}</p>
                                <ul class="space-y-1">
                        ...
                        <template x-if="panel.timeline?.length">
                            <div class="mt-4 border-t border-slate-200 pt-3">
                                <p class="mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ __('Timeline') }}</p>
                                <ul class="space-y-2">
