        ...
        <form method="GET" class="flex flex-wrap items-end gap-3">
            <div>
                ...
                <select name="period_id" class="erp-input mt-1">
                    <option value="">{{ __('All posted') }}</option>
                    @foreach ($periods as $period)
                        <option value="{{ $period->id }}" @selected(($filters['period_id'] ?? null) == $period->id)>{{ $period->code }} — {{ $period->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <label class="flex items-center gap-2 text-sm">
                ...
                {{ __('Include zero-balance accounts') }}
            </label>
