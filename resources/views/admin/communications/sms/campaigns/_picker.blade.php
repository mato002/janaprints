...
>
    <div class="flex flex-wrap items-center justify-between gap-2">
        <div>
            <p class="text-xs font-semibold text-slate-500">{{ __('Pick recipients') }}</p>
            <p class="text-xs text-slate-500">
                ...
                {{ __('shown') }}
                ·
                {{ __('Leave empty to include everyone matching the filters on the right.') }}
            </p>
        </div>
        <div class="flex gap-2">
            <button type="button" class="erp-btn erp-btn--ghost erp-btn--sm" @click="selectAllVisibleRecipients()">{{ __('Select all visible') }}</button>
            <button type="button" class="erp-btn erp-btn--ghost erp-btn--sm" @click="clearSelectedRecipients()">{{ __('Clear') }}</button>
        </div>
    </div>
    ...
    <div class="max-h-40 overflow-y-auto rounded-md border border-erp-border divide-y divide-erp-border bg-white">
        ...
        <p x-show="visibleRecipients.length === 0" class="px-3 py-4 text-center text-xs text-slate-500">
            {{ __('No people with phone numbers for this source.') }}
        </p>
    </div>
