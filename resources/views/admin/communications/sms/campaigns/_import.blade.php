...
>
    <div class="flex flex-wrap items-start justify-between gap-2">
        <div>
            <p class="text-xs font-semibold text-slate-500">{{ __('Import phone list') }}</p>
            <p class="text-xs text-slate-500">
                {{ __('Upload a CSV/TXT file or paste numbers. One phone per line, or name,phone.') }}
            </p>
        </div>
        <a
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        <div>
            <label class="erp-label text-xs">{{ __('Upload CSV / TXT') }}</label>
            <input
                type="file"
            ...
            >
            <p class="mt-1 text-[11px] text-slate-500">{{ __('Headers like name,phone are detected automatically.') }}</p>
        </div>
    ...
    <div>
        <label class="erp-label text-xs">{{ __('Paste or edit numbers') }}</label>
        <textarea
    ...
    <template x-if="importSummary">
        <div class="rounded border border-erp-border bg-slate-50 px-3 py-2 text-xs space-y-1">
            <p>
                ...
                {{ __('valid numbers ready to send') }}
...
<div
    ...
    x-cloak
>
    <p class="text-xs font-semibold text-slate-500">{{ __('Manual numbers') }}</p>
    <p class="text-xs text-slate-500">{{ __('Enter one phone number per line.') }}</p>
    <textarea
        class="erp-input w-full font-mono text-sm"
        rows="4"
