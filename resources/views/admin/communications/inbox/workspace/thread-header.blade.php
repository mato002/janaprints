...
<header class="shared-inbox__thread-header">
    <div class="flex items-start gap-3">
        <div class="shared-inbox__thread-header-avatar">{{ $initial }}</div>
        ...
        <div class="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
            @if ($slaNeedsAttention)
                <button
                    type="button"
                    @click="$dispatch('open-manage-tab')"
                    class="rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $slaStatus->badgeClass() }}"
                    title="{{ __('View SLA & assignment') }}"
                >
                    {{ $slaStatus->label() }}
                </button>
            @endif
            @if (($media['images'] ?? 0) > 0)
                ...
                >
                    {{ $media['images'] }} {{ __('photos') }}
                </button>
            @endif
            @if ($kpis['unread_count'] > 0)
                <span class="shared-inbox__unread-badge">{{ $kpis['unread_count'] }}</span>
            @endif
            <button
                type="button"
                @click="$dispatch('open-manage-tab')"
                class="shared-inbox__icon-btn"
                title="{{ __('Team, SLA & tags') }}"
