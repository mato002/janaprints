<div class="mb-2 flex flex-wrap items-center gap-1">
    ...
    @endphp
    <a href="{{ $inboxEmbedUrl(route('admin.communications.inbox.index', collect($baseQuery)->except('channel')->all())) }}"
    ...
    @foreach (\App\Enums\InboxMessageChannel::cases() as $ch)
        <a href="{{ $inboxEmbedUrl(route('admin.communications.inbox.index', array_merge($baseQuery, ['channel' => $ch->value]))) }}"
           data-turbo-frame="{{ $inboxTurboFrame }}"
           class="rounded-full px-2 py-0.5 text-[10px] {{ ($channelFilter ?? '') === $ch->value ? 'bg-erp-accent text-white' : 'bg-slate-100 text-slate-600' }}">
            {{ $ch->label() }}
        </a>
    @endforeach
    <a href="{{ $inboxEmbedUrl(route('admin.communications.inbox.index', array_merge($baseQuery, ['channel' => 'note']))) }}"
       data-turbo-frame="{{ $inboxTurboFrame }}"
    ...
    <a href="{{ $inboxEmbedUrl(route('admin.communications.inbox.index', array_merge($baseQuery, ['channel' => 'erp']))) }}"
       data-turbo-frame="{{ $inboxTurboFrame }}"
