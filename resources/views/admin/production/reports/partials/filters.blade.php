@props(['filters', 'branches', 'customers', 'job_cards', 'production_types'])

<x-admin.card class="mb-6">
    <form method="GET" action="{{ route('admin.production.reports.index') }}" data-turbo-frame="erp-main" class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <input type="hidden" name="tab" value="{{ $filters['tab'] ?? 'job_profitability' }}">

        <div>
            <label class="text-[11px] text-slate-500" for="from_date">{{ __('From') }}</label>
            <input type="date" id="from_date" name="from_date" value="{{ $filters['from_date'] }}" class="erp-input mt-1 w-full">
        </div>
        <div>
            <label class="text-[11px] text-slate-500" for="to_date">{{ __('To') }}</label>
            <input type="date" id="to_date" name="to_date" value="{{ $filters['to_date'] }}" class="erp-input mt-1 w-full">
        </div>
        <div>
            <label class="text-[11px] text-slate-500" for="branch_id">{{ __('Branch') }}</label>
            <select id="branch_id" name="branch_id" class="erp-input mt-1 w-full">
                <option value="">{{ __('All branches') }}</option>
                @foreach ($branches as $branch)
                    <option value="{{ $branch->id }}" @selected(($filters['branch_id'] ?? null) == $branch->id)>{{ $branch->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="text-[11px] text-slate-500" for="customer_id">{{ __('Customer') }}</label>
            <select id="customer_id" name="customer_id" class="erp-input mt-1 w-full">
                <option value="">{{ __('All customers') }}</option>
                @foreach ($customers as $customer)
                    <option value="{{ $customer->id }}" @selected(($filters['customer_id'] ?? null) == $customer->id)>{{ $customer->company_name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="text-[11px] text-slate-500" for="production_type">{{ __('Product / Department') }}</label>
            <select id="production_type" name="production_type" class="erp-input mt-1 w-full">
                <option value="">{{ __('All types') }}</option>
                @foreach ($production_types as $type)
                    <option value="{{ $type->value }}" @selected(($filters['production_type'] ?? null) === $type->value)>{{ str($type->value)->headline() }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="text-[11px] text-slate-500" for="job_card_id">{{ __('Job') }}</label>
            <select id="job_card_id" name="job_card_id" class="erp-input mt-1 w-full">
                <option value="">{{ __('All jobs') }}</option>
                @foreach ($job_cards as $jobCard)
                    <option value="{{ $jobCard->id }}" @selected(($filters['job_card_id'] ?? null) == $jobCard->id)>{{ $jobCard->job_card_number }}</option>
                @endforeach
            </select>
        </div>
        <div class="md:col-span-2">
            <label class="text-[11px] text-slate-500" for="search">{{ __('Job Number Search') }}</label>
            <input
                type="search"
                id="search"
                name="search"
                value="{{ $filters['search'] ?? '' }}"
                placeholder="{{ __('Search job number…') }}"
                class="erp-input mt-1 w-full"
            >
        </div>
        <div class="flex items-end">
            <button type="submit" class="erp-btn-primary w-full sm:w-auto">{{ __('Apply filters') }}</button>
        </div>
    </form>
</x-admin.card>
