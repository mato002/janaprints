<div class="@if ($embeddedInFloor ?? false) space-y-3 @endif">
    @if ($embeddedInFloor ?? false)
        <div>
            <h2 class="text-sm font-semibold text-erp-primary">{{ __('Finished goods outputs') }}</h2>
            <p class="text-xs text-slate-600">{{ __('Completion records posted from production jobs.') }}</p>
        </div>
    @endif
