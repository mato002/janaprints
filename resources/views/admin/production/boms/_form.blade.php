@php
    ...
    $splitLayout = (bool) ($splitLayout ?? false);
...
<div @class(['space-y-4' => ! $splitLayout, 'job-360-materials__split' => $splitLayout]) x-data="bomFormLines(@js($lineRows), @js($materialOptions))">
    @if ($splitLayout)
        <div class="job-360-materials__header">
            <h3 class="mb-3 text-sm font-semibold uppercase tracking-wide text-erp-primary">{{ __('Bill of materials') }}</h3>
        ...
        <div class="job-360-materials__lines">
            ...
            <p class="mb-2 text-xs text-slate-600">{{ __('Paper, ink, and finishing consumed per finished piece.') }}</p>
            @if ($hasSuggestions)
                <p class="mb-2 rounded-md border border-emerald-200 bg-emerald-50 px-2 py-1.5 text-xs text-emerald-950">
                    {{ __('Suggested from the job spec. Change or remove anything that does not apply.') }}
                </p>
            @endif
            <div class="space-y-2">
                <template x-for="(line, index) in lines" :key="index">
                    <div class="grid grid-cols-2 gap-2 rounded border border-slate-200 p-2 sm:grid-cols-6">
                        <div class="col-span-2 sm:col-span-2">
                            <label class="erp-label text-xs">{{ __('Material') }}</label>
                            <select class="erp-input text-sm" :name="`lines[${index}][inventory_item_id]`" x-model="line.inventory_item_id" required>
                        ...
                        <div>
                            <label class="erp-label text-xs">{{ __('Qty / unit') }}</label>
                            <input type="number" step="0.0001" min="0.0001" class="erp-input text-sm" :name="`lines[${index}][quantity_per_unit]`" x-model="line.quantity_per_unit" required>
                        </div>
                        <div>
                            <label class="erp-label text-xs">{{ __('Waste %') }}</label>
                            <input type="number" step="0.01" min="0" max="100" class="erp-input text-sm" :name="`lines[${index}][waste_factor_percent]`" x-model="line.waste_factor_percent">
                        </div>
                        <div class="col-span-2 sm:col-span-2">
                            <label class="erp-label text-xs">{{ __('Notes') }}</label>
                            <div class="flex gap-2">
                                <input type="text" class="erp-input text-sm" :name="`lines[${index}][notes]`" x-model="line.notes">
