<x-admin.card :padding="false">
    <div class="border-b border-erp-border px-4 py-3">
        <h2 class="text-sm font-semibold uppercase tracking-wide text-erp-primary">{{ __('Branch Profitability') }}</h2>
        <p class="mt-0.5 text-xs text-slate-500">{{ __('Comparative branch performance in selected scope') }}</p>
    </div>
    <div class="overflow-x-auto">
        <table class="erp-table w-full text-sm">
            ...
            <tbody>
                @forelse ($dashboard['branch_profitability'] as $row)
                    <tr>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-slate-500">{{ __('No branch profitability data in this scope.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
