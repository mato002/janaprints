...
@section('content')
    <div class="login-scene" aria-hidden="false">
        ...
        <main class="login-scene__main" aria-label="{{ __('Client Login') }}">
            <div class="login-card" data-login-card>
                ...
                @if (! empty($staffSessionActive))
                    <div class="login-alert" role="status">
                        {{ __('You are signed in to the ERP. Submitting this form will switch to a client portal session.') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('client.login') }}" class="login-form" novalidate>
                    @csrf
